...
<header class="header">
    ...
    <?php 
        $sql = $conn->prepare("SELECT * FROM events ORDER BY eventID DESC;");
        $sql->execute();
        $result = $sql->get_result();

        if ($result && mysqli_num_rows($result) > 0) {
            while ($record = mysqli_fetch_assoc($result)) {
                echo "<div class='event'><div class='event-content'><h3>".$record['event_name']."</h3>";
                echo "<p><strong>Issuer: </strong>".$record['event_owner_name']."</p>";
                echo "<p><strong>Date: </strong>".$record['event_date']."</p>";
                echo "<p><strong>Time: </strong>".$record['event_time']."</p>";
                echo "<p class='description'>".$record['event_description']."</p></div>";
                echo "<form action='regevent.php?eventID=".$record['eventID']."' method='post'>
