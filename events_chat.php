...
if(isset($_GET["eventID"])){
    ...
    $sql = $conn->prepare("SELECT * FROM regevents WHERE eventID = ? and userID =?");
    $sql->bind_param("ss", $_GET["eventID"],$_SESSION["userID"]);
    $sql->execute();
    $result = $sql->get_result();
    if($result && mysqli_num_rows($result) > 0){
        $_SESSION["eventID"]=$_GET["eventID"];
        $sql = $conn->prepare("SELECT event_name FROM events WHERE eventID = ?");
        $sql->bind_param("s",$_SESSION["eventID"]);
        $sql->execute();
        $result = $sql->get_result();
        if ($result && mysqli_num_rows($result) > 0){
            $record = mysqli_fetch_assoc($result);
            $_SESSION["event_name"]=$record["event_name"];
...
if(isset($_POST["message"])){
    ...
    $sql = $conn->prepare("INSERT INTO events_chat (senderID,eventID,message) VALUES (?,?,?)");
    $sql->bind_param("sss",$_SESSION["userID"],$_SESSION["eventID"],$escaped_message);
    $sql->execute();
    ...
    <script>
        $(document).ready(function() {
            function updateContent() {
                $('#live-chat').load('events_chat2.php');
 
            }
            setInterval(updateContent, 400); 
        });
    </script>
...
<header class="header">
    ...
    <div class="chat-container">
        <div class="chat-header"><?php echo $_SESSION["event_name"];?></div>
        <div class="chat-messages" id="live-chat">
            <!-- Example messages -->

        </div>
        <div class="chat-input">
        <form action="events_chat.php" method="post">
