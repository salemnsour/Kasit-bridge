...
if($_SESSION["verified"]==false ||$_SESSION["verified2"]==false || $_SESSION["role"]!="company")
{
    header("location: login.php");
    exit;
}
if(isset($_POST["job_name"]) && isset($_POST["job_description"]) && isset($_POST["email"])){
    echo"zzzzzzzzz";
    ...
    $sql = $conn->prepare("INSERT INTO jobs (job_name, job_description, email ,job_owner_id ,job_owner_name) VALUES (?,?,?,?,?)");
    $sql->bind_param("sssss", $job_name, $job_description,$email , $_SESSION["userID"], $_SESSION["user"]);
    if($sql->execute()){
    ...
    <div class="section">
        ...
        <form action="create_job.php" method="post">
            <div class="form-group">
                <label for="event-name">Job Name</label>
                <input type="text" id="job-name" name="job_name" placeholder="Enter the job name">
            </div>
            <form action="" method="post">
                <div class="form-group">
                    <label for="event-name">Email to contact</label>
                    <input type="email" id="email" name="email" placeholder="Enter the email">
                </div>
            <div class="form-group">
                <label for="job-description">Description</label>
                <textarea id="job-description" name="job_description" placeholder="Enter a description of the job"></textarea>
            </div>
